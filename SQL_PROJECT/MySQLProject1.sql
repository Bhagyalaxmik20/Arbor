...
-- Situation -1 : We have hired two employees, Siddharth Gupta (email: siddharth.gupta@example.com, 
...
-- Situation -2 : The Marketing department has been created (DepartmentID: 11) with a budget of 
-- ₹13,00,000, located in Pune, and headed by Rakesh Mehta. Add this department. 
 INSERT INTO Departments (DepartmentName, Location, HeadOfDepartment, AnnualBudget) 
VALUES  
('Marketing', 'Pune', 'Rakesh Mehta',1300000) ;

-- Situation -3 : Three employees have been hired for the Marketing department (DepartmentID: 11): 
...
('Nitin', 'Shah',11, 60000, '2024-01-11',' nitin.shah@example.com,'); 

-- Situation -4 : Two employees have been hired for the HR department (DepartmentID: 1): Riya Sharma 
...
-- situation-5: Sneha Rao (email: sneha.rao@example.com) has joined the Finance department 
-- (DepartmentID: 2) on 2024-02-15 with a salary of ₹60,000. Add this record. 
INSERT  INTO Employees (FirstName, LastName, DepartmentID, Salary, DateOfJoining, 
Email)VALUES 
(' Sneha',' Rao',2,60000,'2024-02-15','sneha.rao@example.com');

-- situation-6: Three employees have been hired for the Legal department (DepartmentID: 7): Aditya 
...
-- Situation -7:The Research department (DepartmentID: 12) has been created with a budget of 
-- ₹20,00,000, located in Hyderabad, and headed by Dr. Meera Joshi. Add this department. 
 INSERT INTO Departments (DepartmentName, Location, HeadOfDepartment, AnnualBudget) 
VALUES  
('Research','Hyderabad','Dr.Meera Joshi',2000000);

-- Situation -8 : Two employees have been hired for the Research department (DepartmentID: 12): Arjun 
...
-- Situation -9 :The Logistics department (DepartmentID: 13) has been created with a budget of 
-- ₹9,00,000, located in Chennai, and headed by Ravi Verma. Add this department. 
INSERT INTO Departments (DepartmentName, Location, HeadOfDepartment, AnnualBudget) 
VALUES 
('Logistics','Chennai','Ravi Verma',900000);

-- Situation -10 : Two employees have been hired for the Logistics department (DepartmentID: 13): Kavita 
...
SELECT * FROM Employees
WHERE YEAR(DateOfJoining) ='2024' AND MONTH(DateOfJoining)='02';
...
SELECT  DepartmentID,COUNT(EmployeeID) AS TotalID FROM Employees
GROUP BY DepartmentID;
...
SELECT DepartmentID , Min(Salary) AS miniSalary FROM Employees
Group by DepartmentID;
-- Situation -7: The finance team wants to review high-salary employees. Find the total salary paid for employees
--  earning more than ₹60,000 in each department.
SELECT DepartmentID ,SUM(Salary) AS TotalSalary FROM Employees
where Salary > 60000
Group BY DepartmentID ; 
-- Situation -8: To monitor departmental growth, count the number of employees who joined in 2024 in each department. 
SELECT DepartmentID ,COUNT(DateOfJoining) AS CountJoined FROM Employees
where DateOfJoining like '2024%'
group by DepartmentID;
...
SELECT CONCAT( EmployeeID,' ',DepartmentID ) AS  UniqueID From Employees;
...
SELECT CONCAT(phoneNumber,IFNULL(Extension, ''))AS ContactNumber FROM Employees;
...
SELECT CONCAT(DepartmentName,'-',Position) AS Notification FROM Employees
JOIN Department d ON e.DepartmentID=d.DepartmentID;
-- Situation -9: To display the employee's work status, concatenate their job title and current project. 
SELECT CONCAT( Jobtitle,'-',CurrentProject) AS WorkStatus FROM Employees;
...
SELECT CONCAT('2025',FirstName) AS ReferenceCode From Employees;
-- *****************************************************************
-- 5] Situational Questions on the UPDATE Clause 
-- Situation -1: As part of an organizational review, we need to update the salary of all employees who are part of the 
-- IT department (DepartmentID: 3). The salary of all employees in this 
...
select * from Employees;
-- Situation -2: After the recent promotion evaluation, it has been decided that the 
-- employee with EmployeeID 101 should be promoted to a "Senior Developer" role. Update their job title 
-- accordingly and make sure the position is reflected correctly in the employee records. 
UPDATE Employees
SET JobTitle = 'Senior Developer'
WHERE EmployeeID = 101;

-- Situation -3: The HR team has informed us of a change in the contact information of employee with EmployeeID 120. The new contact number for this employee should be updated to 
-- "9876543210". Ensure that the correct phone number is reflected in the system for any future communications. 
-- Situation -4: The company has recently implemented a new email domain. All employees who joined after January 1, 2023, need their email addresses updated to reflect the new domain 
...
where DateOfJoining > '2023-01-01' AND EmployeeID IS NOT NULL;
-- Situation -5: It has come to our attention that there was an error in the naming of the HR department. The department name for
--  DepartmentID 5 needs to be updated from "HR" to 
...
-- Situation -6: As part of a company-wide salary review process, all employees who are currently earning below ₹50,000 
-- need to receive a salary increase of ₹5,000. This update should only 
-- affect employees with a salary below the threshold and should apply the increase uniformly to each of their records. 
UPDATE Employees
SET Salary = Salary + 5000
WHERE Salary < 50000;
-- Situation -7: The Marketing department has recently moved to a new office in a different city. The location of the Marketing department 
-- (DepartmentID: 11) needs to be updated to reflect this 
...
-- Situation -9: EmployeeID 110, who was previously working on a different project, has now been assigned to "Project Alpha" after the 
-- successful completion of their prior assignment.-- Please update the project assignment for this employee to reflect this new responsibility. 
UPDATE Employees
SET Project = 'Project Alpha'
WHERE EmployeeID = 110;

-- Situation -10: The employee with EmployeeID 103 had a delayed start date due to personal reasons. We need to update their joining
--  date to the correct date, which is 2024-03-15, to ensure 
-- the records accurately reflect the employee's actual start date with the company.
UPDATE Employees
SET JoiningDate = '2024-03-15'
...
DELETE FROM Employees
WHERE Email IS NULL OR ContactNumber IS NULL; 
